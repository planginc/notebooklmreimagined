...
export async function POST(
  ...
  try {
    ...
    try {
      ...
      if (type === 'data_table') {
        resultContent = await generateDataTable(truncatedContent, enhancedInstructions || undefined)
        costUsd = 0.005
      } else if (type === 'report') {
        resultContent = await generateReport(truncatedContent, report_type || 'briefing_doc', enhancedInstructions || undefined)
        costUsd = 0.01
